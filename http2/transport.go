...
func (t *Transport) newClientConn(c net.Conn, addr string, singleUse bool) (*ClientConn, error) {
	...
	if t.EnablePush == 1 && t.PushHandler != nil {
...
var errRequestCanceled = errors.New("github.com/SandwichDev/net/http: request canceled")
...
func (b transportResponseBody) Read(p []byte) (n int, err error) {
	...
	if cs.bytesRemain != -1 {
		if int64(n) > cs.bytesRemain {
			...
			if err == nil {
				err = errors.New("github.com/SandwichDev/net/http: server replied with more than declared Content-Length; truncated")
